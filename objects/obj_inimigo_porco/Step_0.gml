...
if (estado == "normal")
{
	...
	if (_chao)
	{
		...
		//Se eu bater na parede, eu mudo a direção
		...
		//Se eu não posso cair, então quadno eu cair eu mudo de direção
		if (pode_cair == false)
		if (place_meeting(x + (velh * 10), y + 1, obj_chao) == false)
		{
			//Se eu não posso cair e não estou tocando no chão na minha frente
			//Então eu mudo a direção
			velh *= -1;
		}
	}
	else 
	{ //Se não estou no chão, eu aplico a gravidade
		velv += grav;
